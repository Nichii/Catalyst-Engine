...
struct XGameUiPlayerPickerInfo
{
    ...
    _Field_z_ const char* promptText;
    uint32_t selectFromPlayersCount;
    _Field_size_(selectFromPlayersCount) const uint64_t* selectFromPlayers;
    uint32_t preSelectedPlayersCount;
    _Field_size_(preSelectedPlayersCount) const uint64_t* preSelectedPlayers;
    uint32_t minSelectionCount;
    uint32_t maxSelectionCount;
};

typedef void CALLBACK XGameUiShowPlayerProfileCardUiCallback(
    _In_ XGameUiCallbackHandle callbackHandle,
...
typedef void CALLBACK XGameUiShowPlayerPickerUiCallback(
    _In_ XGameUiCallbackHandle callbackHandle,
    _In_ XTaskQueueHandle queue,
    _In_ const XGameUiPlayerPickerInfo* info,
    _In_opt_ void* context
    );

typedef void CALLBACK XGameUiShowSendGameInviteUiCallback(
    _In_ XGameUiCallbackHandle callbackHandle,
    _In_ XTaskQueueHandle queue,
    _In_ XUserHandle requestingUser,
    _In_z_ const char* sessionConfigurationId,
...
typedef void CALLBACK XGameUiShowAchievementsUiCallback(
    _In_ XGameUiCallbackHandle callbackHandle,
    _In_ XTaskQueueHandle queue,
    _In_ XUserHandle requestingUser,
    _In_ uint32_t titleId,
...
typedef void CALLBACK XGameUiShowMultiplayerActivityGameInviteUiCallback(
    _In_ XGameUiCallbackHandle callbackHandle,
    _In_ XTaskQueueHandle queue,
    _In_ XUserHandle requestingUser,
    _In_opt_ void* context
...
typedef void CALLBACK XGameUiShowMessageDialogUiCallback(
    _In_ XGameUiCallbackHandle callbackHandle,
    _In_ XTaskQueueHandle queue,
    _In_z_ const char* titleText,
    _In_z_ const char* contentText,
...
typedef void CALLBACK XGameUiShowErrorDialogUiCallback(
    _In_ XGameUiCallbackHandle callbackHandle,
    _In_ XTaskQueueHandle queue,
    _In_ HRESULT errorCode,
    _In_opt_z_ const char* serviceContext,
    _In_opt_ void* context
    );

typedef void CALLBACK XGameUiShowTextEntryUiCallback(
    _In_ XGameUiCallbackHandle callbackHandle,
    _In_ XTaskQueueHandle queue,
    _In_opt_z_ const char* titleText,
    _In_opt_z_ const char* descriptionText,
...
STDAPI XGameUiSetUiCallbacks(
    _In_ const XGameUiUiCallbacks* callbacks,
    _In_ bool useSystemUiIfAvailable
    ) noexcept;

STDAPI XGameUiSetMessageDialogUiResponse(
    _In_ XGameUiCallbackHandle callbackHandle,
    _In_ XGameUiMessageDialogButton response
    ) noexcept;

STDAPI XGameUiSetPlayerPickerUiResponse(
    _In_ XGameUiCallbackHandle callbackHandle,
    _In_ uint32_t playerCount,
    _In_reads_opt_(playerCount) const uint64_t* players
    ) noexcept;
