...
STDAPI_(bool) XUserIsSignOutPresent() noexcept;

STDAPI XUserSignOutAsync(
    _In_ XUserHandle user,
    _Inout_ XAsyncBlock* async
    ) noexcept;

STDAPI XUserSignOutResult(
    _Inout_ XAsyncBlock* async
    ) noexcept;
...
// Platform implementation api and callbacks

typedef struct XUserPlatformOperationToken* XUserPlatformOperation;

enum class XUserPlatformOperationResult
{
    Success = 0,
    Failure = 1,
    Canceled = 2,
...
enum class XUserPlatformSpopOperationResult
{
    SignInHere = 0,
    SwitchAccount = 1,
    Failure = 2,
...
typedef void (XUserPlatformRemoteConnectShowPromptEventHandler)(
    ...
    _In_reads_bytes_(dataSize) void const* qrCode
    );
...
STDAPI XUserPlatformRemoteConnectSetEventHandlers(
    _In_opt_ XTaskQueueHandle queue,
...
// TODO
typedef void CALLBACK XUserFindControllerForUserUiCallback(
    _In_opt_ void* context,
    _In_ XUserHandle user
    );
