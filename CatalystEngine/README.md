...
* To support remote deployment to handheld devices, the template enables the property `CopyCppRuntimeToOutputDir` to copy the Visual C++ Runtime files for app-local deployment. For Debug builds, this copies the Debug version of the Visual C++ CRT. Note that in cases where a game uses a mix of Debug and Retail-built DLLs, you will need to deploy both the Debug and Release version. This can be implemented in the vcxproj by adding this target:

```xml
  <Target Name="MixDebugAndReleaseCRT" BeforeTargets="_CopyFilesMarkedCopyLocal"
    Condition="'$(CopyCppRuntimeToOutputDir)' == 'true'">
    <ItemGroup>
      <_CppRuntimeFilesDirs Include="$(CppRuntimeFilesPath)" />
      <ReferenceCopyLocalPaths Include="%(_CppRuntimeFilesDirs.Identity)\**\*.dll" />
    </ItemGroup>
  </Target>
```
